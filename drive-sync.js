...
async function _multipart(method, path, meta, content) {
await *ensureToken();
const boundary = ’scull*’ + Math.random().toString(36).slice(2);
const body = [
`--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${JSON.stringify(meta)}`,
`--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n${content}`,
`--${boundary}--`
...
const panel = document.createElement(‘div’);
panel.style.cssText = `background:#1a1a2e;border:1px solid #334;border-radius:10px; padding:18px;max-width:420px;width:92%;max-height:80vh; overflow-y:auto;color:#e0e0e0;font-family:monospace;`;
...
const btn = document.createElement(‘button’);
btn.textContent = label;
btn.style.cssText = `display:block;width:100%;text-align:left;padding:9px 12px; margin-bottom:6px;background:#252540;border:1px solid #334; border-radius:6px;color:#e0e0e0;font-family:monospace; font-size:.85rem;cursor:pointer;`;
btn.onmouseenter = () => btn.style.background = ‘#2e2e58’;
btn.onmouseleave = () => btn.style.background = ‘#252540’;
btn.onclick = () => { overlay.remove(); onSelect(f.id, label); };
panel.appendChild(btn);
...
const cancel = document.createElement(‘button’);
cancel.textContent = ‘Cancel’;
cancel.style.cssText = `display:block;width:100%;padding:8px;margin-top:8px; background:transparent;border:1px solid #556;border-radius:6px; color:#888;cursor:pointer;font-family:monospace;`;
cancel.onclick = () => overlay.remove();
panel.appendChild(cancel);
...
1. SCRIPT TAG — add before </body> in scull.html AND analyser.html:
   ...
   OR inline the entire file contents into each HTML file.
1. INIT — in your DOMContentLoaded / app init block:
   
   DriveSync.init().catch(console.warn); // line 309
1. DRIVE BUTTON HTML — add wherever suits (settings panel, header):
...
border-radius:50%;background:#27ae60;display:inline-block;margin-left:4px;"></span>
   ...
   The button label auto-toggles between “⊞ Drive” and “⊟ Drive”.
   The dot goes green when linked.
1. addSess() — replace the existing function with the one above.
   No other changes to saveSess() / loadSess() needed.
1. ANALYSER — add a “Load from Drive” button alongside the CSV import:
   
   <button onclick="loadFromDrive()">⊞ Load from Drive</button>
   
   Then ensure renderAnalysis(data, label) is whatever your analyser calls
   after parseCSV() returns — just swap parseCSV(text) for sessionToAnalyserFmt(session).
1. GOOGLE CLOUD CONSOLE — confirm these are set:
   Authorised JavaScript origins:
   https://kstg4cnkwh-netizen.github.io
   Authorised redirect URIs:
   (none needed for implicit/token flow)
